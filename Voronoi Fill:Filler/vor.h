...
namespace filler
{
        ...
        animation vorCUSTDFS(PNG& img, double density, double fadeFactor, int frameFreq);

        animation vorCUSTBFS(PNG& img, double density, double fadeFactor, int frameFreq);
...
vector<center> randSample(PNG& img, double density);
    ...
    animation vor(PNG& img, double density, colorPicker& fillColor,
                   int frameFreq);
    bool allEmpty(vector<bool> hasElements);
    bool isValidPoint(int xval, int yval, double dist, int levelOrder, vector<vector<bool>> processed, PNG& img);
    double calcDist(int xval, int yval, center c);
}
